...
\imagem{Esquemático do problema inverso}{
...
\node at (-1,-1) {\small Entrada inicial};
\node at (3.5,-1) {\small Iteração};
\node at (4,2.5) {\small Saída modelo};
\draw  (-1,2) -- (0.5,2);
\node at (-1,2.5) {\small Saída esperada};
\node (v2) at (5.5,-2) {};
\node at (v2) {\small Demais parâmetros};
\draw  (v2) edge (5.5,-1);
\end{tikzpicture}
}
\label{fig:pi-exemplo}
}{O autor}{fig:pi-exemplo}{}{Esquemático com o problema inverso.}
